...
function ConversationScreen({
  ...
  const showMessages = () => {
    // Nếu front-end đang loading tin nhắn, hiển thị các tin nhắn được lấy từ Next SSR (truyền từ [id].tsx)
    if (messagesLoading) {
    ...
    // Nếu front-end đã hoàn tất loading tin nhắn, nên là ta sẽ có messagesSnapshot
    if (messagesSnapshot) {
  ...
  const addMessageToDbAndUpdateLastSeen = async () => {
    // cập nhật trang thái đăng nhập cuối trong 'users' collection
    await setDoc(
      doc(db, "users", loggedInUser?.email as string),
      {
        lastSeen: serverTimestamp(),
      },
      { merge: true } // chỉ cập nhật những gì đã thay đổi
    );
  ...
  return (
    <>
      ...
      <StyledMessageContainer>
        ...
        {/* Tự động scroll xuống dưới cuối khi mà tin nhắn mới được gửi đi */}
